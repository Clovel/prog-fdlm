...
| Auth table IDs | UUID (via `advanced.database.generateId` + `uuid` columns), matching Spec 1. |
...
| Column | Type | Notes |
| --- | --- | --- |
| id | uuid | PK; value from `generateId` (`crypto.randomUUID()`) |
...
betterAuth({
  ...
  advanced: {
    ...
    backgroundTasks: { handler: (promise) => waitUntil(promise) },  // reliable email send on serverless
  },
...
- `input: false` on `role` blocks the field from being set through any auth request; only server-side code (seed now, Spec 3 endpoints later) writes it.
...
- `waitUntil` is imported from the platform (`@vercel/functions`) in production; locally a pass-through is fine.
...
2. Write `src/auth/roles.ts`, `gravatar.ts`, `email.ts`, then `config.ts`.
3. **Spike:** confirm BetterAuth internal user-creation API against the installed version.
